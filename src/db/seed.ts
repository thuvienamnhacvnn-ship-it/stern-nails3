 ...
 *
 * It lives here rather than in `scripts/` because two callers need it: the
 * command a developer runs against the local database, and the bootstrap that
 * builds a database from nothing when the site starts on a host with no disk
 * to keep one on. It truncates first, so running it twice gives one catalogue
 * rather than two.
 */
export async function seed(db: PgliteDatabase<typeof schema>): Promise<void> {
